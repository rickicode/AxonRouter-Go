...
## [Unreleased]
### Added
- **Media provider registry expansion** — added first-class support for media providers in `internal/executor/registry.go`: `fal`, `black-forest-labs`, `assemblyai`, `edge-tts`, and `cartesia` now route through the OpenAI executor; existing `elevenlabs` and `deepgram` prefixes retained and promoted in the model catalog. Added canonical entries and aliases in `internal/provider/aliases.go`, static catalog entries in `internal/models/models.json`, and seeded `provider_types` rows in `internal/db/migrations.go` with appropriate `service_kinds`. Wired catalog lookups in `internal/api/handlers/v1/models.go`, `internal/api/handlers/admin/models.go`, and `providerCatalogKeys` so `/v1/models` and admin model listing expose the new providers. Added unit tests in `internal/executor/registry_test.go`, `internal/models/catalog_test.go`, and `internal/provider/aliases_test.go` covering registration and model resolution without collisions.
