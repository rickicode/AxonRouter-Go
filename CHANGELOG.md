...
## [Unreleased]
### Added
- **Headroom external compression proxy** — new `internal/headroom` package ports the 9Router Headroom-style external compression service to AxonRouter-Go. Detects and compresses common tool outputs (`git diff`, `git log`, `git status`, `grep`, `find`/`tree`, build logs, search results) and Anthropic `tool_result` content blocks before upstream forwarding. Provides a pure-Go in-process server on `127.0.0.1:9123` (auto-fallback port) or an external endpoint configured via `AXON_HEADROOM_ENDPOINT` / dashboard settings. Configuration env vars: `AXON_HEADROOM_ENABLED`, `AXON_HEADROOM_TIMEOUT_MS`, `AXON_HEADROOM_MAX_PAYLOAD_BYTES`. Integrated into `compressRequestBody` fail-open so local compression still applies if Headroom is off or errors. Admin `/api/admin/settings/compression` reads/writes `headroom` fields and `/compression/metrics` exposes `headroom_status`, `headroom_endpoint`, and counters (`headroom_total`, `headroom_bytes_saved`, `headroom_errors`). Adds unit tests for detect, compress, server/client roundtrip, and handler integration (`internal/headroom/...`, `internal/api/handlers/v1/request_optimization_headroom_test.go`). Documentation in `docs/headroom.md` and spec additions in `openspec/specs/compression/spec.md`.
