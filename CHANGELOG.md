...
## [Unreleased]
### Added
- **Chinese/regional OpenAI-compatible providers** — added `qwen`, `alicode`, `kimi-coding`, `iflow`, `volcengine-ark`, and `hunyuan` prefixes to `internal/executor/registry.go` using the shared OpenAI executor and default OpenAI-compatible error translator. Extended the existing `minimax` catalog with `minimax-text-01` and `minimax-vision`, and extended `kimi` with `kimi-k2.7-coder`. Added `kimi-coding` as a dedicated prefix for the Kimi coding endpoint. Added canonical provider entries in `internal/provider/aliases.go` and model catalog entries in `internal/models/models.json`. Added registry and catalog regression tests in `internal/executor/registry_test.go` and `internal/models/catalog_test.go`. Updated README.md provider table and `openspec/specs/provider/spec.md`.
