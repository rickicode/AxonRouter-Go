...
type ProxyGroupHandler struct{ db *sql.DB }

func NewProxyGroupHandler(database *sql.DB) *ProxyGroupHandler { return &ProxyGroupHandler{db: database} }
...
func (h *ProxyGroupHandler) List(c *gin.Context) {
	...
	where := "1=1"
	args := []any{}
	if v := c.Query("is_active"); v != "" { where += " AND is_active = ?"; args = append(args, boolQuery(v)) }
	if v := c.Query("mode"); v != "" { where += " AND mode = ?"; args = append(args, v) }
	var total int
	...
	defer rows.Close()
	items := []db.ProxyGroup{}
	for rows.Next() { if g, ok := scanGroup(rows); ok { items = append(items, g) } }
	pages := total / perPage
...
func (h *ProxyGroupHandler) Get(c *gin.Context) {
	...
	if !ok { c.JSON(http.StatusNotFound, gin.H{"error": "proxy group not found"}); return }
...
func (h *ProxyGroupHandler) Create(c *gin.Context) {
	...
	if mode == "" { mode = "roundrobin" }
	if mode != "roundrobin" && mode != "sticky" { c.JSON(http.StatusBadRequest, gin.H{"error": "mode must be roundrobin or sticky"}); return }
	...
	active := true
	if _, ok := req["isActive"]; ok { active = asBool(req["isActive"]) }
...
func (h *ProxyGroupHandler) Update(c *gin.Context) {
	id := c.Param("id")
	if _, ok := h.get(id); !ok { c.JSON(http.StatusNotFound, gin.H{"error": "proxy group not found"}); return }
	var req map[string]any
	if c.ShouldBindJSON(&req) != nil { c.JSON(http.StatusBadRequest, gin.H{"error": "invalid JSON"}); return }
	sets := []string{}
	args := []any{}
	...
	if _, ok := req["mode"]; ok { mode := asString(req["mode"]); if mode != "roundrobin" && mode != "sticky" { c.JSON(http.StatusBadRequest, gin.H{"error": "mode must be roundrobin or sticky"}); return }; add("mode", mode) }
	if _, ok := req["stickyLimit"]; ok { n := asInt(req["stickyLimit"], 1); if n < 1 { n = 1 }; add("sticky_limit", n) }
	if _, ok := req["strictProxy"]; ok { add("strict_proxy", boolToInt(asBool(req["strictProxy"]))) }
	if _, ok := req["isActive"]; ok { add("is_active", boolToInt(asBool(req["isActive"]))) }
	if _, ok := req["proxyPoolIds"]; ok { ids, err := h.poolIDs(req["proxyPoolIds"]); if err != nil { c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()}); return }; raw, _ := json.Marshal(ids); add("proxy_pool_ids", string(raw)) }
...
func (h *ProxyGroupHandler) Delete(c *gin.Context) {
	...
	if _, err := h.db.Exec("DELETE FROM proxy_groups WHERE id = ?", id); err != nil { c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()}); return }
...
func (h *ProxyGroupHandler) poolIDs(v any) ([]string, error) {
	items, ok := v.([]any)
	if !ok || len(items) == 0 { return []string{}, nil }
	ids := make([]string, 0, len(items))
	for _, item := range items {
		id := strings.TrimSpace(asString(item))
		if id == "" { continue }
		var exists int
		if err := h.db.QueryRow("SELECT 1 FROM proxy_pools WHERE id = ?", id).Scan(&exists); err != nil { return nil, err }
		ids = append(ids, id)
	}
	return ids, nil
}
...
func scanGroup(row rowScanner) (db.ProxyGroup, bool) {
	...
	var ids string
	if err := row.Scan(&g.ID, &g.Name, &g.Mode, &g.StickyLimit, &strict, &ids, &active, &g.CreatedAt, &g.UpdatedAt); err != nil { return g, false }
	g.StrictProxy = strict != 0
	g.IsActive = active != 0
	_ = json.Unmarshal([]byte(ids), &g.ProxyPoolIDs)
	if g.ProxyPoolIDs == nil { g.ProxyPoolIDs = []string{} }
	return g, true
}
