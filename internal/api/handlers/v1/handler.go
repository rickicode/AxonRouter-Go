...
func (h *Handler) getConnection(ctx context.Context, provider string, modelID string) (*Connection, error) {
	...
	if len(conns) > 0 {
		start := h.pickStartIndex(provider, modelID, len(conns))
		...
		for i := 0; i < bound; i++ {
			...
			if conn, ok := h.tryPickConnection(ctx, conns[idx], provider, modelID); ok {
				return conn, nil
			}
		}
...
func (h *Handler) getConnectionFallback(ctx context.Context, provider, modelID string) (*Connection, error) {
	...
	for _, cs := range candidates {
		if conn, ok := h.tryPickConnection(ctx, cs, provider, modelID); ok {
	...
	for _, cs := range candidates {
		if conn, ok := h.tryPickConnectionFallback(ctx, cs.ID, provider, modelID); ok {
...
func (h *Handler) tryPickConnection(ctx context.Context, cs *connstate.ConnectionState, provider, modelID string) (*Connection, bool) {
	if cs.IsInCooldown() {
		return nil, false
	}
	if modelID != "" && cs.IsModelInCooldown(modelID) {
...
func (h *Handler) orderCandidates(provider, modelID string, candidates []*connstate.ConnectionState) []*connstate.ConnectionState {
	if len(candidates) <= 1 {
		return candidates
	}
