...
func (h *Handler) getConnection(ctx context.Context, provider string, modelID string) (*Connection, error) {
	...
	if len(conns) > 0 {
		start := h.pickStartIndex(provider, len(conns))
...
func (h *Handler) orderCandidates(provider string, candidates []*connstate.ConnectionState) []*connstate.ConnectionState {
	if len(candidates) <= 1 {
		return candidates
	}
