...
func TestProviderTypeCategoryAndServiceKinds(t *testing.T) {
	...
	cases := []struct {
		...
		{"cf", "apikey", []string{"llm", "embedding", "image", "tts", "stt"}},
		{"cx", "oauth", []string{"llm", "image"}},
		{"oc", "no-auth", []string{"llm"}},
		{"claude", "apikey", []string{"llm", "imageToText"}},
		{"mimocode", "no-auth", []string{"llm"}},
		{"glm", "apikey", []string{"llm"}},
		{"minimax", "apikey", []string{"llm", "image", "imageToText", "webSearch", "tts"}},
		{"kimi", "apikey", []string{"llm", "webSearch"}},
		{"mistral", "apikey", []string{"llm", "imageToText", "embedding"}},
		{"cerebras", "apikey", []string{"llm"}},
		{"together", "apikey", []string{"llm", "embedding"}},
		{"fireworks", "apikey", []string{"llm", "embedding"}},
