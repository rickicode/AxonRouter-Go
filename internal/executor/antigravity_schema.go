...
// CleanJSONSchemaForAntigravity transforms a JSON schema to be compatible with the Antigravity API.
...
// removeKeywords removes all occurrences of the specified keywords from the JSON schema.
...
func removePlaceholderFields(jsonStr string) string {
	for _, p := range findPaths(jsonStr, "_") {
		...
		if req.IsArray() {
			filtered := make([]string, 0, len(req.Array()))
	...
	for _, p := range findPaths(jsonStr, "reason") {
		...
		if gjson.Get(jsonStr, p+".description").String() != placeholderReasonDescription {
			continue
		}
		jsonStr, _ = sjson.Delete(jsonStr, p)
		reqPath := joinPath(parentPath, "required")
...
// convertRefsToHints converts $ref properties to description hints.
...
// convertConstToEnum converts schema "const" constraints to equivalent "enum" values.
...
// convertEnumValuesToStrings normalizes enum values and types to strings as required by Gemini.
...
// addEnumHints appends a compact allowed-values hint to property descriptions.
...
// addAdditionalPropertiesHints records strict object shape requirements in descriptions.
...
// moveConstraintsToDescription preserves unsupported constraints as description hints.
...
// mergeAllOf flattens allOf compositions into a single object schema.
...
// flattenAnyOfOneOf collapses oneOf/anyOf unions to the best single schema plus hints.
func flattenAnyOfOneOf(jsonStr string) string {
	for _, key := range []string{"anyOf", "oneOf"} {
		...
		for _, p := range paths {
			...
			if len(allTypes) > 1 {
				selected = appendHintRaw(selected, "Accepts: "+strings.Join(allTypes, " | "))
			}
...
// selectBest picks the richest schema from a oneOf/anyOf list for downstream processing.
...
// flattenTypeArrays converts type arrays like ["string","null"] to a single type and hints.
func flattenTypeArrays(jsonStr string) string {
	...
	for _, p := range paths {
		...
		hasNull := false
		nonNullTypes := make([]string, 0, len(res.Array()))
...
// removeUnsupportedKeywords deletes Gemini/Antigravity-rejected schema keywords.
...
// removeExtensionFields removes x-* extension fields from the JSON schema.
...
// cleanupRequiredFields removes required entries that no longer exist as properties.
func cleanupRequiredFields(jsonStr string) string {
	for _, p := range findPaths(jsonStr, "required") {
		...
		valid := make([]string, 0, len(req.Array()))
		for _, r := range req.Array() {
			key := r.String()
...
// addEmptySchemaPlaceholder adds required placeholder fields to empty object schemas.
func addEmptySchemaPlaceholder(jsonStr string) string {
	...
	for _, p := range paths {
		...
		if propsVal.IsObject() && !hasRequiredProperties && parentPath != "" {
...
func mergeDescriptionRaw(schemaRaw, parentDesc string) string {
	...
	case childDesc == parentDesc:
		return schemaRaw
	default:
		updated, _ := sjson.SetBytes([]byte(schemaRaw), "description", fmt.Sprintf("%s (%s)", parentDesc, childDesc))
		return string(updated)
	}
}
