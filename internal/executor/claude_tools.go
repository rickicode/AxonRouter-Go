...
// TitleCase names. Anthropic uses tool-name fingerprinting; renaming avoids
// extra-usage billing and OAuth rejection.
...
// Currently empty; all known tools are mapped instead.
var oauthToolsToRemove = map[string]bool{}

// remapOAuthToolNames renames third-party tool names to Claude Code equivalents
// and drops tools without an official counterpart. It returns the modified body
// plus a per-request reverse map used to restore original names in the response.
...
// reverseRemapOAuthToolNames restores original tool names for non-streaming responses.
...
// reverseRemapOAuthToolNamesFromStreamLine restores original tool names for SSE
// stream lines.
...
// jsonPayload extracts the JSON payload from an SSE line (after optional "data:" prefix).
