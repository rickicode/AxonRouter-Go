// Package common contains shared utilities used by request/response translators.
package common
...
// GeminiTokenCountJSON returns a Gemini-style token count JSON payload.
...
// ClaudeInputTokensJSON returns a Claude-style input token count JSON payload.
...
// JoinRawArray joins raw JSON items into a single JSON array.
...
// SSEEventData builds a raw SSE event frame with the given event name and payload.
// The returned bytes contain the "event:" and "data:" lines but no trailing
// newlines; callers should add "\n\n" to terminate the frame.
...
// AppendSSEEventString appends an SSE event frame built from string payload to out.
...
// AppendSSEEventBytes appends an SSE event frame built from byte payload to out.
